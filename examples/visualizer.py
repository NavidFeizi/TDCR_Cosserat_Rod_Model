...
df = pd.read_csv(os.path.join(os.path.dirname(__file__), "output", "backbone.csv"), delimiter=",")

P = df[["x", "y", "z"]].to_numpy().T
...
plt.pause(0.1)
plt.savefig(
    os.path.join(os.path.dirname(__file__), "output", "tdcr_cpp_results.png"),
    format="png",
    dpi=600,
)
plt.show(block=True)
